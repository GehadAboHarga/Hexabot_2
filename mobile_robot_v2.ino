...
ros::Publisher pub_odom( "/odometry", &odometry_msg);

char base_link[] = "/base_link";
char odom[] = "/odom";
double theta=0, x=0, y=0;
float intens[6]={99999, 99999, 99999 ,99999, 99999,99999};
...
void t_data1();
void from_dxdy_to_xytheta(float pos_l, float pos_r);
long range_time;
...
void messageCb( const geometry_msgs::Twist& msg){
   ...
   omega_r = ((speed_lin/wheel_rad) + ((speed_ang*wheel_sep)/(2.0*wheel_rad)))*48.66;
   omega_l = ((speed_lin/wheel_rad) - ((speed_ang*wheel_sep)/(2.0*wheel_rad)))*48.66;
   if (omega_r >= 255) omega_r = 255;
   if (omega_r <= -255) omega_r = -255;
   if (omega_l >= 255) omega_l = 255;
   if (omega_l <= -255) omega_l = -255;
...
void loop() {
  ...
  odometry_msg.pose.pose.position.y = y;
   odometry_msg.pose.pose.orientation = t.transform.rotation;
...
void parse_encoders(String command_encoders1){
  ...
  encoderL = part2_encoders.toFloat();
  encoderR = part1_encoders.toFloat();
}
